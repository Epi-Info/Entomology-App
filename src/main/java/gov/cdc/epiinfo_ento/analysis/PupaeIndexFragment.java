...
public class PupaeIndexFragment extends Fragment {

    private OnFragmentInteractionListener mListener;
    private LinearLayout content;

    public PupaeIndexFragment() {

    }

    public static PupaeIndexFragment newInstance(String param1, String param2) {
        PupaeIndexFragment fragment = new PupaeIndexFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_pupae_index, container, false);
        content = v.findViewById(R.id.rate_content);

        SetupGadget();

        if (mListener != null) {
            mListener.onFragmentInteraction(getString(R.string.dash_pupae_index));
        }
    ...
    private void SetupGadget() {
        ...
        dbHelper.open();
        Cursor c = dbHelper.getPupaeIndex();

        if (c.moveToFirst()) {
            do {

                double index = 100.0 * c.getDouble(c.getColumnIndexOrThrow("TotalPupae")) / c.getDouble(c.getColumnIndexOrThrow("TotalHouses"));
