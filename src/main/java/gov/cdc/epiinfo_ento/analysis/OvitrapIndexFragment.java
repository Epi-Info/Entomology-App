...
public class OvitrapIndexFragment extends Fragment {

    private OnFragmentInteractionListener mListener;
    private LinearLayout content;

    public OvitrapIndexFragment() {

    }

    public static OvitrapIndexFragment newInstance(String param1, String param2) {
        OvitrapIndexFragment fragment = new OvitrapIndexFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_ovitrap_index, container, false);
        content = v.findViewById(R.id.rate_content);

        SetupGadget();

        if (mListener != null) {
            mListener.onFragmentInteraction(getString(R.string.dash_ovitrap_index));
        }
    ...
    private void SetupGadget() {
        ...
        EpiDbHelper dbHelper = new EpiDbHelper(getActivity(), new FormMetadata("EpiInfoEntomology/Questionnaires/_Trapping.xml", getActivity()), "_trapping");
        dbHelper.open();
        Cursor c = dbHelper.getOvitrapIndex();

        if (c.moveToFirst()) {
            do {

                double a = c.getDouble(c.getColumnIndexOrThrow("PositiveTraps"));
                double b = c.getDouble(c.getColumnIndexOrThrow("TotalTraps"));

                double index = 100.0 * c.getDouble(c.getColumnIndexOrThrow("PositiveTraps")) / c.getDouble(c.getColumnIndexOrThrow("TotalTraps"));
