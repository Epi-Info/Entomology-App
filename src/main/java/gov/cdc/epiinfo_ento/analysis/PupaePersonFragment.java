...
public class PupaePersonFragment extends Fragment {

    private OnFragmentInteractionListener mListener;
    private LinearLayout content;

    public PupaePersonFragment() {

    }

    public static PupaePersonFragment newInstance(String param1, String param2) {
        PupaePersonFragment fragment = new PupaePersonFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_pupae_person, container, false);
        content = v.findViewById(R.id.rate_content);

        SetupGadget();

        if (mListener != null) {
            mListener.onFragmentInteraction(getString(R.string.dash_ppp));
        }
    ...
    private void SetupGadget() {
        ...
        dbHelper.open();
        Cursor c = dbHelper.getPupaePerPerson();

        if (c.moveToFirst()) {
            do {

                double a = c.getDouble(c.getColumnIndexOrThrow("TotalPupae"));
                double z = c.getDouble(c.getColumnIndexOrThrow("Population"));

                double index = 1.0 * c.getDouble(c.getColumnIndexOrThrow("TotalPupae")) / c.getDouble(c.getColumnIndexOrThrow("Population"));
