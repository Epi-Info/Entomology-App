...
import static gov.cdc.epiinfo_ento.R.drawable.circle;

public class OvitrapDensityFragment extends Fragment {

    private OnFragmentInteractionListener mListener;
    private LinearLayout content;

    public OvitrapDensityFragment() {

    }

    public static OvitrapDensityFragment newInstance(String param1, String param2) {
        OvitrapDensityFragment fragment = new OvitrapDensityFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_ovitrap_density, container, false);
        content = v.findViewById(R.id.rate_content);

        SetupGadget();

        if (mListener != null) {
            mListener.onFragmentInteraction(getString(R.string.dash_ovitrap_density));
        }
    ...
    private void SetupGadget() {
        ...
        EpiDbHelper dbHelper = new EpiDbHelper(getActivity(), new FormMetadata("EpiInfoEntomology/Questionnaires/_TrappingLab.xml", getActivity()), "_trappinglab");
        dbHelper.open();
        Cursor c = dbHelper.getOvitrapDensity();

        if (c.moveToFirst()) {
            do {

                double a = c.getDouble(c.getColumnIndexOrThrow("TotalEggs"));
                double b = c.getDouble(c.getColumnIndexOrThrow("TotalTraps"));

                double index = 1.0 * c.getDouble(c.getColumnIndexOrThrow("TotalEggs")) / c.getDouble(c.getColumnIndexOrThrow("TotalTraps"));
