...
public class ContainerIndexFragment extends Fragment {

    private OnFragmentInteractionListener mListener;
    private LinearLayout content;

    public ContainerIndexFragment() {

    }

    public static ContainerIndexFragment newInstance(String param1, String param2) {
        ContainerIndexFragment fragment = new ContainerIndexFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_container_index, container, false);
        content = v.findViewById(R.id.rate_content);

        SetupGadget();

        if (mListener != null) {
            mListener.onFragmentInteraction(getString(R.string.dash_container_index));
        }
    ...
    private void SetupGadget() {
        ...
        dbHelper.open();
        Cursor c = dbHelper.getContainerIndex();

        if (c.moveToFirst()) {
            do {

                double a = c.getDouble(c.getColumnIndexOrThrow("PositiveContainers"));
                double b = c.getDouble(c.getColumnIndexOrThrow("TotalContainers"));

                double index = 100.0 * c.getDouble(c.getColumnIndexOrThrow("PositiveContainers")) / c.getDouble(c.getColumnIndexOrThrow("TotalContainers"));
