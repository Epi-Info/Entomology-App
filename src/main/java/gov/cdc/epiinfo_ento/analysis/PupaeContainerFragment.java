...
public class PupaeContainerFragment extends Fragment {

    private OnFragmentInteractionListener mListener;
    private LinearLayout content;

    public PupaeContainerFragment() {

    }

    public static PupaeContainerFragment newInstance(String param1, String param2) {
        PupaeContainerFragment fragment = new PupaeContainerFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_pupae_container, container, false);
        content = v.findViewById(R.id.rate_content);

        SetupGadget();

        if (mListener != null) {
            mListener.onFragmentInteraction(getString(R.string.dash_ppc));
        }
    ...
    private void SetupGadget() {
        ...
        dbHelper.open();
        Cursor c = dbHelper.getPupaePerContainer();

        if (c.moveToFirst()) {
            do {

                double a = c.getDouble(c.getColumnIndexOrThrow("TotalPupae"));
                double z = c.getDouble(c.getColumnIndexOrThrow("TotalContainers"));

                double index = 1.0 * c.getDouble(c.getColumnIndexOrThrow("TotalPupae")) / c.getDouble(c.getColumnIndexOrThrow("TotalContainers"));
