	...
	public FormMetadata(String viewXmlFile, Activity container)
	{
		try
		{
			...
			{ 
				...
				try
				{
					Concept = obj_view_list.item(0).getAttributes().getNamedItem("Concept").getNodeValue();
					ConceptPlural = obj_view_list.item(0).getAttributes().getNamedItem("ConceptPlural").getNodeValue();
				}
				catch (Exception ex)
				{
